...
import {
    openKelasSessionAction,
    closeKelasSessionAction,
    deleteKelasSessionAction,
    type OpenKelasSessionResult,
...
export function KelasSessionPanel({
    domain,
    openSession,
    todaysSession,
}: {
    ...
    const [openState, openAction, openPending] = useActionState<OpenKelasSessionResult, FormData>(
        (_, formData) => openKelasSessionAction(domain, formData),
        { ok: true },
    );
    const [closeState, closeAction, closePending] = useActionState<CloseKelasSessionResult, FormData>(
        () => closeKelasSessionAction(domain, openSession?.id ?? ""),
        { ok: true },
    );
    const [deleteState, deleteAction, deletePending] = useActionState<DeleteKelasSessionResult, FormData>(
        () => deleteKelasSessionAction(domain, (openSession ?? todaysSession)?.id ?? ""),
        { ok: true },
    );
        ...
        return (
            <div className="rounded-lg border bg-card text-card-foreground shadow-sm p-6 space-y-3">
                <div className="flex items-center justify-between">
                    <h2 className="text-lg font-semibold">Sesi Kelas Aktif</h2>
    ...
    if (todaysSession && todaysSession.status === "closed") {
        return (
            <div className="rounded-lg border bg-card text-card-foreground shadow-sm p-6 space-y-3">
                <div className="flex items-center justify-between">
                    <h2 className="text-lg font-semibold">Sesi Kelas</h2>
