...
export async function manageStudentLifecycleAction(domain: string, formData: FormData) {
  const principal = await enforceMasterDataAccess(domain, "write"), parsed = parseStudentLifecycleForm(formData); if (!parsed) finish(domain, "invalid-input");
  const result = await (parsed.operation === "transition" ? service.transition(principal, parsed.id, parsed) : parsed.operation === "correct-graduation" ? service.correctGraduation(principal, parsed.id, { ...parsed, toStatus: parsed.toStatus === "graduated" ? "active" : parsed.toStatus }) : parsed.operation === "archive" ? service.archive(principal, parsed.id, parsed) : service.reactivate(principal, parsed.id, parsed)).catch(() => null);
  if (!result) finish(domain, "error", parsed.id); finish(domain, studentResultCode(result), parsed.id);
}
