...
export async function archiveSchoolPersonAction(domain: string, formData: FormData): Promise<never> {
  const principal = await enforceTenantMasterDataOperation(domain, "people.archive"), personId = String(formData.get("personId") ?? "").trim(), origin = String(formData.get("origin") ?? ""), selected = String(formData.get("selected") ?? "").trim(), reason = String(formData.get("reason") ?? "").trim(), expectedVersion = Number.parseInt(String(formData.get("expectedVersion") ?? ""), 10);
