...
export async function GET(_request: Request, context: { params: Promise<{ domain: string; kind: string }> }) {
  const { domain, kind } = await context.params;
  await enforceTenantMasterDataOperation(domain, "people-imports.template.download");
  ...
  return new Response(bytes, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="template-${kind}-v1.0.0.xlsx"`,
