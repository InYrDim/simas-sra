...
import { db } from "@/db";
import { schoolPerson, studentAudit, studentLifecyclePeriod, studentProfile, studentRelationship, tenant, user } from "@/db/schema";
...
async function listWith(executor: Pick<typeof db, "select">, tenantId: string): Promise<StudentRecord[]> {
  const [rows, blockers] = await Promise.all([executor.select({ person: schoolPerson, student: studentProfile, accountEnabled: user.emailVerified }).from(studentProfile).innerJoin(schoolPerson, and(eq(schoolPerson.tenantId, studentProfile.tenantId), eq(schoolPerson.id, studentProfile.personId))).leftJoin(user, and(eq(user.tenantId, tenantId), eq(user.id, schoolPerson.accountUserId))).where(eq(studentProfile.tenantId, tenantId)).orderBy(asc(schoolPerson.normalizedName), asc(studentProfile.id)), executor.select().from(studentRelationship).where(and(eq(studentRelationship.tenantId, tenantId), eq(studentRelationship.active, true)))]);
  return rows.map(({ person, student, accountEnabled }) => ({ person: { ...person, accountActive: Boolean(accountEnabled) }, student, classGroupName: null, archiveBlockers: blockers.filter((item) => item.studentId === student.id) }));
}
...
export const studentMasterDataStore: StudentMasterDataStore = {
  ...
  transaction(tenantId, work) { return db.transaction(async (tx) => { await tx.execute(sql`SELECT ${tenant.id} FROM ${tenant} WHERE ${tenant.id} = ${tenantId} FOR UPDATE`); return work({
    ...
    async closeLifecyclePeriod(id, endedAt) { const result = await tx.update(studentLifecyclePeriod).set({ endedAt }).where(and(eq(studentLifecyclePeriod.tenantId, tenantId), eq(studentLifecyclePeriod.id, id), sql`${studentLifecyclePeriod.endedAt} IS NULL`)); return result[0].affectedRows === 1; },
