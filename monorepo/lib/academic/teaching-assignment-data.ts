...
async function endpoints(executor: Pick<typeof db, "select">, tenantId: string, input: { teacherProfileId: string; subjectId: string; classGroupId: string; academicYearId: string }): Promise<TeachingAssignmentEndpoint> {
  const [[teacher], [subjectRow], [group], [year]] = await Promise.all([
     executor.select({ id: teacherProfile.id, tenantId: teacherProfile.tenantId, active: sql<boolean>`${teacherProfile.status} = 'active'`, archived: teacherProfile.archived }).from(teacherProfile).where(and(eq(teacherProfile.tenantId, tenantId), eq(teacherProfile.id, input.teacherProfileId))).limit(1),
...
export const teachingAssignmentStore: TeachingAssignmentStore = {
  async actor(tenantId, userId) {
    const [row] = await db.select({ id: user.id, lifecycle: tenantAccountSecurity.lifecycle }).from(user).leftJoin(tenantAccountSecurity, eq(tenantAccountSecurity.userId, user.id)).where(and(eq(user.tenantId, tenantId), eq(user.id, userId))).limit(1);
    return Boolean(row && (row.lifecycle === null || row.lifecycle === "active"));
  },
