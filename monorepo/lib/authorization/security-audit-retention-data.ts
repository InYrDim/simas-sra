...
import { contextColumns, type SecurityContext } from "@/lib/authorization/security-command";
...
export async function saveSecurityAuditRetentionPolicy(input: Readonly<{
  ...
  await db.insert(securityAuditRetentionPolicy).values({
...
export async function openSecurityAuditLegalHold(input: Readonly<{
  ...
  await db.insert(securityAuditLegalHold).values({
    id,
    ...contextColumns(input.context),
    securityContextKind: input.context.kind,
    contextId: input.context.contextId,
...
export async function releaseSecurityAuditLegalHold(input: Readonly<{ context: SecurityContext; caseId: string; releasedAt?: Date }>): Promise<boolean> {
  assertContext(input.context);
  const at = input.releasedAt ?? new Date();
  const updated = await db.update(securityAuditLegalHold).set({ state: "released", releasedAt: at }).where(and(
...
export async function issueSecurityAuditRetentionCertificate(input: Readonly<{
  ...
  return db.transaction(async (database) => {
    ...
    const [tenantRow] = input.context.kind === "tenant"
      ...
      : [];
    const tenantDeleted = input.tenantDeleted ?? tenantRow?.operationalStatus === "closed";
