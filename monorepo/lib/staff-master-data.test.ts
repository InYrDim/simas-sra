...
test("enumerates archive blockers, warns for active accounts, and reactivates without restoring history", async () => {
  ...
  const historyCount = data.periods.length; const reactivated = await catalog.reactivate(principal, created.record.staff.id, { reason: "Profil diperlukan kembali", expectedVersion: 3 }); assert.equal(reactivated.ok, true); assert.equal(data.staffs[0].status, "ended"); assert.equal(data.periods.length, historyCount); assert.equal(data.blockers[0].active, false);
});
