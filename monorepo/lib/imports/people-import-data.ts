...
async function validationAuthority(connection: mysql.PoolConnection, input: { tenantId: string; actorId?: string; rolloutEpoch?: string | null }) {
  if (!input.actorId) return false;
  const [rows] = await connection.query<mysql.RowDataPacket[]>("SELECT t.operational_status,t.trial_ends_at,t.settings,u.tenant_id,u.tenant_role,sec.lifecycle account_lifecycle,rollout.epoch rollout_epoch,(SELECT COUNT(*) FROM school_admin_authority a WHERE a.tenant_id=t.id AND a.user_id=u.id AND a.authority_state='active') active_authority_count,(SELECT COUNT(DISTINCT p.permission_key) FROM tenant_role_assignment ra JOIN tenant_role r ON r.tenant_id=ra.tenant_id AND r.id=ra.role_id JOIN tenant_role_permission p ON p.tenant_id=r.tenant_id AND p.role_id=r.id WHERE ra.tenant_id=t.id AND ra.user_id=u.id AND ra.state='active' AND r.lifecycle='active' AND p.permission_key='people-imports.revisions.import') import_permission_count FROM tenant t JOIN user u ON u.tenant_id=t.id AND u.id=? LEFT JOIN tenant_account_security sec ON sec.tenant_id=t.id AND sec.user_id=u.id LEFT JOIN tenant_rbac_rollout rollout ON rollout.tenant_id=t.id WHERE t.id=? FOR UPDATE", [input.actorId, input.tenantId]);
  const row = rows[0];
  if (!row || row.tenant_id !== input.tenantId || row.operational_status !== "active" || (row.trial_ends_at && new Date(row.trial_ends_at) <= new Date()) || (row.account_lifecycle && row.account_lifecycle !== "active") || !isTenantFeatureEnabled(row.settings, "masterDataImportValidation")) return false;
  if (input.rolloutEpoch === null || (input.rolloutEpoch !== undefined && String(row.rollout_epoch ?? "") !== input.rolloutEpoch)) return false;
  return row.tenant_role === "school-admin" && Number(row.active_authority_count) > 0 || Number(row.import_permission_count) > 0;
}
