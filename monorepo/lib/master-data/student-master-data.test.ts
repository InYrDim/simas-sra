...
import { createStudentMasterDataService, type SchoolPerson, type StudentAudit, type StudentLifecyclePeriod, type StudentMasterDataStore, type StudentProfile, type StudentRelationshipBlocker } from "@/lib/master-data/student-master-data";
  ...
  const store: StudentMasterDataStore = {
    ...
    async transaction(tenantId, work) {
      ...
      try { return await work({
        ...
        async listGuardians(studentId) { return blockers.filter((item) => item.tenantId === tenantId && item.studentId === studentId && item.kind === "orangtua").map((item) => ({ ...structuredClone(item), phone: (item as StudentRelationshipBlocker & { phone?: string | null }).phone ?? null })); },
        async saveGuardian(value) { if (value.tenantId !== tenantId) throw new Error("Cross-Tenant guardian write denied"); if (!value.id) { const id = `guardian-${Date.now()}`; blockers.push({ ...value, id, active: value.active } as StudentRelationshipBlocker & { phone?: string | null }); return { ok: true, id }; } const index = blockers.findIndex((item) => item.tenantId === tenantId && item.id === value.id); if (index < 0) return { ok: false, code: "not-found" }; blockers[index] = { ...blockers[index], ...value }; return { ok: true, id: value.id }; },
        async deleteGuardian(id) { const index = blockers.findIndex((item) => item.tenantId === tenantId && item.id === id); if (index < 0) return false; blockers.splice(index, 1); return true; },
