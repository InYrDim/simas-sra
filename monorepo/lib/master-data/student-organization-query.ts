import type { StudentOrganization } from "@/lib/master-data/student-organization";
import { normalizeMasterDataQuery, type MasterDataSearchParams } from "@/lib/master-data/master-data-workspace";

export function queryStudentOrganizations(records: readonly StudentOrganization[], raw: MasterDataSearchParams) {
  ...
  const filtered = records
    ...
    .filter((item) => !query.filters.location?.length || query.filters.location.includes(item.secretariatLocationId ? "assigned" : "unassigned"))
    .filter((item) => !needle || `${item.name}${item.code}${item.abbreviation ?? ""}`.toLocaleLowerCase("id-ID").replace(/\s+/g, "").includes(needle))
