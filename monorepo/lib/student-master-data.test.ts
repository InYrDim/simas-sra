...
test("enumerates archive blockers, warns for active accounts, and reactivates without restoring history", async () => {
  ...
  const historyCount = data.periods.length; const reactivated = await catalog.reactivate(principal, created.record.student.id, { reason: "Profil diperlukan kembali", expectedVersion: 3 }); assert.equal(reactivated.ok, true); assert.equal(data.students[0].status, "transferred"); assert.equal(data.periods.length, historyCount); assert.equal(data.blockers[0].active, false);
});
