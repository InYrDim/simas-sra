...
test("route adapter parses only the Mata Pelajaran transport contract", () => {
  ...
  form.set("id", "subject-1"); form.set("version", "4"); form.set("code", "MAT-01"); form.set("name", "Matematika"); form.append("educationLevels", "SMP"); form.set("description", "Dasar"); form.set("ignoredTeacherId", "teacher-1");
  assert.deepEqual(parseSubjectForm(form, "SMA"), { id: "subject-1", version: 4, input: { code: "MAT-01", name: "Matematika", educationLevels: ["SMA"], description: "Dasar" } });
});

test("route adapter rejects malformed identity/version and maps safe result codes", () => {
  const form = new FormData();
  form.set("version", "NaN");
  assert.equal(parseSubjectForm(form, "SMA"), null);
