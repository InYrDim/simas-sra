...
Implementation notes: execution and validation workers now recheck Tenant/account/authority/entitlement and rollout state, use fenced leases, and atomically persist audit, success, outcome, and transactional-outbox records. MySQL coverage verifies concurrent workers, duplicate submission, revocation, epoch change, partial failure, and outbox cardinality. Centralized evaluator parity for every retry path and crash/audit-failure race fixtures remain before final resolution.
